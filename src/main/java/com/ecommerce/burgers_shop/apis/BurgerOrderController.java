...
public class BurgerOrderController {
    ...
    @Autowired
    private BurgerOrderRepositoryImpl repo;

    @Autowired
    private OrderMessagingService messageService;

    @GetMapping(produces = "application/json")
    public Iterable<BurgerOrder> allOrders() {
        return repo.findAll();
    }

    @PostMapping(consumes = "application/json")
    @ResponseStatus(HttpStatus.CREATED)
    public BurgerOrder postOrder(@RequestBody BurgerOrder order) {
        this.messageService.sendOrder(order);
    ...
    public BurgerOrder patchOrder(@PathVariable("orderId") Long orderId, @RequestBody BurgerOrder patch) {

        BurgerOrder order = repo.findById(orderId).get();
